...
class MaskInfo(NamedTuple):
  ...
  * num_heads when there is more than one unique mask for each head in the
  ...
  If data_next and block_mask are None, then this communicates to the kernel
  that we are operating with a fully dynamic mask. In this case all blocks are
  processed and masked.
  ...
  Attributes:
    ...
    q_sequence: A i32[q_sequence_length] NumPy array. When using causal masking,
      ...
      causal this is just np.arange(q_sequence_length). These will be shuffled
...
def _get_mask_info_for_shard(
  ...
  for idx in np.ndindex(blocked_shape):
    if is_dkv:
      kv_index, h_index, q_index = idx
      chunk_idx = (h_index, q_index, kv_index)
      data_dim = 2
    else:
      chunk_idx = idx
      data_dim = 2
...
def _process_mask(
  ...
  is_one_mask_dynamic = any(
      isinstance(mask.masks[h], mask_lib.DynamicMask) for h in range(head_count)
  )

  if is_one_mask_dynamic:
    all_masks_are_dynamic = all(
        isinstance(mask.masks[h], mask_lib.DynamicMask)
        for h in range(head_count)
    )

    if not all_masks_are_dynamic:
      raise ValueError(
          'Mixing dynamic masks with non-dynamic masks across heads is not'
          ' supported'
      )

    assert isinstance(mask.masks[0], mask_lib.DynamicMask)

    # When using a dynamic mask, there is no possiblity to skip any blocks, so
    # no need for data_next and block_mask.
    return (
        MaskInfo(
            data_next=None,
            mask_next=None,
            ...
            q_sequence=mask.masks[0].q_sequence,
        ),
        mask.masks[0].mask_function,
    )
  ...
  # TODO(b/300590271): In the future we could think of a dynamic mapping of
  ...
  block_id_to_block_coords: Dict[int, List[Tuple[int, int, int]]] = (
  ...
  if (
    ...
    if q_seq_shards == 1:
      ...
      for (
          ...
          block_mask_shards, data_next_shards, mask_next_shards, strict=True
      ):
